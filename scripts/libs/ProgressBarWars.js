$.fn.extend({ProgressBarWars: function(opciones) {
					...
					$("#bar"+theidProgressBarWars).animate({width: opciones.percentage+"%"},opciones.time);
					this.moveTo = function(ntamanio) {
					$("#bar"+$(this).attr("id")).animate({width:ntamanio+"%"},opciones.time);
					};
	return this;			 
	}
});